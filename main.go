...
func EnsureSessionInfo(ctx context.Context) *SessionInfo {
	...
	if sessionInfo == nil {
		zap.L().Info("Creating session.", zap.String("sessionId", sessionId))
		...
		go func() {
			...
			zap.L().Info("Closing session.", zap.String("sessionId", sessionId))
			sessionLock.Lock()
			defer sessionLock.Unlock()
...
func main() {
	...
	go ssh.ListenAndServe(":2222", nil, ssh.HostKeyFile("serverkey"))
	...
	http.ListenAndServe(":12345", nil) // TODO option
}
...
func (h *ForwardedTCPHandler) HandleSSHRequest(ctx ssh.Context, srv *ssh.Server, req *gossh.Request) (bool, []byte) {
	...
	case "tcpip-forward":
		...
		domain := subDomain + ".localhost.localdomain" // TODO option
